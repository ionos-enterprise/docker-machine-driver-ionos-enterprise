...
release:
	...
	tar --remove-files -cvzf bin/$(name)-$(version)-linux-amd64.tar.gz -C bin $(name)
	GOOS=darwin GOARCH=amd64 GOGC=off CGOENABLED=0 go build -i -o bin/$(name) ./bin
	tar --remove-files -cvzf bin/$(name)-$(version)-darwin-amd64.tar.gz -C bin $(name)
	GOOS=windows GOARCH=amd64 GOGC=off CGOENABLED=0 go build -i -o bin/$(name).exe ./bin
	tar --remove-files -cvzf bin/$(name)-$(version)-windows-amd64.tar.gz -C bin $(name).exe
